...
| 标志 | 长参数               | 描述               | 默认值  |
| ---- | -------------------- | ------------------ | ------- |
| `-i` | `--interface`        | 监控的网络接口     | 必需    |
| `-l` | `--listen`, `--http` | Web 服务器监听地址 | `:9092` |
| `-c` | `--config`           | 配置文件路径       | -       |
...
| 标志  | 长参数            | 描述                 | 默认值                   |
| ----- | ----------------- | -------------------- | ------------------------ |
| `-s`  | `--sync-interval` | 数据同步间隔（秒）   | `1`                      |
| -     | `--flow-ttl`      | 不活跃流量超时（秒） | `300`                    |
| `-gc` | `--gc-ttl`        | 清理间隔（秒）       | **自动** = flow-ttl/5    |
| -     | `--client-ttl`    | 客户端缓存时长（秒） | **自动** = flow-ttl\*0.4 |

> 💡 **智能默认值**: `--gc-ttl` 和 `--client-ttl` 会根据 `--flow-ttl` 自动计算。大多数用户只需设置 `--flow-ttl` 即可。

### 过滤选项

| 标志     | 长参数          | 描述                            | 默认值   |
| -------- | --------------- | ------------------------------- | -------- |
| `--lan`  | `--monitor-lan` | 监控局域网流量（禁用过滤）      | `false`  |
| `--cidr` | `--local-cidr`  | 要忽略的本地 CIDR（可多次指定） | 自动检测 |

### 高级选项

| 标志 | 长参数       | 描述                                      | 默认值 |
| ---- | ------------ | ----------------------------------------- | ------ |
| `-x` | `--xdp-mode` | XDP 模式 (auto, generic, driver, offload) | `auto` |
...
ProBPF 内置 Prometheus metrics 导出功能，可与 Grafana 集成。
...
- `probpf_global_download_bps` - 总下载速率
- `probpf_global_upload_bps` - 总上传速率
- `probpf_global_active_connections` - 活跃连接数
- `probpf_global_active_devices` - 在线设备数

**设备级指标：**

- `probpf_device_download_bps{mac, name}` - 设备下载速率
- `probpf_device_upload_bps{mac, name}` - 设备上传速率
- `probpf_device_bytes_total{mac, name, direction}` - 设备流量统计

**协议统计：**

- `probpf_protocol_bytes_total{protocol, direction}` - 按协议统计流量

### Grafana Dashboard

项目包含预配置的 Grafana Dashboard (`grafana.json`)，包含：
...
- **详情视图**:
  ...
  - **全局统计**: 历史总流量计数器。
  - **流列表**: 所有活跃连接的实时列表，包含协议、远程 IP 和端口。
- **控制**:
  - **自动刷新**: 暂停/恢复 实时更新。
  - **主题**: 切换 深色/浅色 模式。
  - **IP 信息**: 选择首选 IP 地理位置提供商 (ipinfo.io, ipapi.is, censys.io 等)。
